...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding=1, bias=False) # in_channels, out_channels, kernel_size, stride, padding, dilation, goups, bias
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size, padding=1, bias=False) # in_channels should be out_channels to take out_channels from previous Conv2d layer
    ...
    def forward(self, x, fmap_dict=None, prefix=""):
        ...
        out_calc = out + shortcut

        if fmap_dict is not None:
            fmap_dict[f"{prefix}.conv"] = out_calc

        out = torch.relu(out_calc)
        if fmap_dict is not None:
